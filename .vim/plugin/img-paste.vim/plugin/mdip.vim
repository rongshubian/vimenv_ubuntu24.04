...
function! s:SaveFileTMPWSL(imgdir, tmpname) abort
    ...
    if tmpfile =~ "mnt"
        ...
        "don't know why , my wsl path with version number
        let tmpfile = '\\\\wsl\$\\Ubuntu-22.04'.tmpfile
    endif

    " modify by zxk
    "orig cmd
    "let clip_command = 'powershell.exe -nologo -noprofile -noninteractive -sta "Add-Type -Assembly PresentationCore;'.
    " todo : replace by pwsh 7
    " let absolute_cmd_path = '/mnt/c/Program\ Files/PowerShell/7/' 
    " let clip_command = absolute_cmd_path . 'pwsh.exe -nologo -noprofile -noninteractive -sta "Add-Type -Assembly PresentationCore;'.
     
    let absolute_cmd_path = '/mnt/c/Windows/System32/WindowsPowerShell/v1.0/' 
    let clip_command = absolute_cmd_path . 'powershell.exe -nologo -noprofile -noninteractive -sta "Add-Type -Assembly PresentationCore;'.
    ...
    " echomsg 'zxk  SaveFileTMPWSL. ' . clip_command
    " echomsg result
    if result ==# "good"
        " echomsg 'zxk  savegood. '
        return tmpfile
    else
        " echomsg 'zxk  notegood. '
        return 1
    endif
endfunction
